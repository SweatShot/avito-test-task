...
- **`getAds`** — получить список объявлений с поддержкой пагинации, фильтров, поиска и сортировки.  
- **`getAllIds`** — получить полный список ID объявлений (для перехода «пред/след»).  
- **`getAdById`** — получить детальную информацию об объявлении, включая характеристики, продавца и историю модерации.  
- **`approveAd`** — одобрить объявление.  
- **`rejectAd`** — отклонить объявление, указав причину и комментарий (опционально).  
- **`requestChanges`** — отправить объявление на доработку с указанием причины.  

---

### Модератор (`moderatorsApi`)

- **`getCurrentModerator`** — получить информацию о текущем авторизованном модераторе, включая статистику и права доступа.  

---

### Статистика (`statsApi`)

- **`getSummaryStats`** — получить сводную статистику модерации за выбранный период.  
- **`getActivityChart`** — получить данные для графика активности модератора по дням.  
- **`getDecisionsChart`** — получить распределение решений (одобрено / отклонено / доработка) для круговой диаграммы.  
- **`getCategoriesChart`** — получить статистику количества проверок по категориям.  
